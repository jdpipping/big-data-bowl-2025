...
Safety1_PreSnapMovement_ByPlay <- MergedData_PreSnapFrames %>% filter(nflId == pre_snap_safety_1) %>%
  ...
  summarize(PreSnap_Frames = n(), down = max(down), distance = max(yardsToGo), Team = max(club),
            ...
            Safety1_Max_PreSnap_X_DistFromBall = max(X_DistFromBall),
            Safety1_Max_PreSnap_Y_AbsDistFromBall = max(Y_AbsDistFromBall), Safety1_Max_PreSnap_Tot_DistFromBall = max(TotDistFromBall),
...
Safety2_PreSnapMovement_ByPlay <- MergedData_PreSnapFrames %>% filter(nflId == pre_snap_safety_2) %>%
  ...
  summarize(PreSnap_Frames = n(), down = max(down), distance = max(yardsToGo), Team = max(club),
            ...
            Safety2_Max_PreSnap_X_DistFromBall = max(X_DistFromBall),
            Safety2_Max_PreSnap_Y_AbsDistFromBall = max(Y_AbsDistFromBall), Safety2_Max_PreSnap_Tot_DistFromBall = max(TotDistFromBall),
