...
# PART 1: General importing and cleansing of data
...
BallSnap_Events <- tracking_combined %>% filter(event %in% c("ball_snap", "snap_direct")) %>%
  group_by(gameId, playId, nflId, displayName) %>%
...
# PART 2: Data engineering (largely JP's code)
...
# Same concept as previously established X_NetDistFromBall_Rank_BySide, but this refers to only at the time of the snap
...
# Next step is to mutate each frame with the ID of the QB on that play
# But some plays have multiple, such as Taysom Hill plays: View(MergedData %>% filter(gameId == 2022091100, playId == 301, PlayerSideOfBall == "offense", frameId == 1))
...
# PART 3: Justin's Voronoi/vectorization code, along with projecting future distances
...
# Vectorized code, along with explanations
...
# Part 5 is modeling code: see separate R document for that (neural network)
...
# Part 6: Aggregating frames into plays
...
# Here's how to export to a CSV
write.csv(Stats_ByFullPlay_All9Weeks, "processed-data/Stats_ByFullPlay_All9Weeks.csv")